...
final class AdminSettings {
	...
	public function register_api_routes() {
		register_rest_route( 'admin-settings/v1', "/{$this->page_slug}/update", array(
			'methods'             => 'POST',
	...
	public function enqueue_scripts( $hook_suffix ) {
		...
		wp_enqueue_style( "{$this->page_slug}-settings-app", $this->plugin_url . 'admin-settings/build/static/css/index.css', array(), $this->plugin_version, );
		...
		$localize_data = array_merge( $this->page_data,
			array(
				...
				'settingsApiUrl' => get_rest_url( false, "admin-settings/v1/{$this->page_slug}/update" ),
			)
		);
		wp_enqueue_script( "{$this->page_slug}-settings-app", $this->plugin_url . 'admin-settings/build/static/js/index.js', array( 'wp-element' ), $this->plugin_version, true );
		wp_localize_script( "{$this->page_slug}-settings-app", 'reactData', $localize_data );
	}

	public function render_settings_page() {
		echo '<div id="admin-settings-root"></div>';
	}
